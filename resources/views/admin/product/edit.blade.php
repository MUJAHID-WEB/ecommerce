...
<div class="row page-titles">
    <div class="col-md-5 align-self-center">
        ...
        @include('admin.includes.breadcrumb', ['title'=>'Product Edit'])
        
        <a href="{{Route('product.index')}}" class="btn btn-info d-none d-lg-block m-l-15"><i class="fa fa-plus-circle"></i> ALL INFO</a>
        </div>
    </div>
</div>
...
<div class="col-lg-12">
    <div class="card">
        <div class="card-body">
            <form method="POST" class="update_form product_insert_form" action="{{Route('product.update', $product->id)}}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-body">
                            ...
                            <div class="form-group">
                                ...
                                <select name="status" id="status" class="form-control">
                                    ...
                                    @foreach ($status as $item)
                                        <option {{$product->status==$item->id ? 'selected' : ''}} value="{{$item->serial}}">{{$item->name}}</option>  
                                    @endforeach
                                </select> 
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="main_category_id" id="main_category" class="form-control">
                                    ...
                                    @foreach ($main_categories as $item)
                                        <option {{$product->main_category()->first()->id == $item->id ? 'selected' : ''}} value="{{$item->id}}">{{$item->name}}</option>  
                                    @endforeach
                                </select> 
                                {{-- <select name="main_category_id" id="main_category" class="form-control">
                                    <option>Select ...</option> 
                                ...
                                <select name="category_id" id="category" multiple class="form-control select2 select2-multiple">
                                    ...
                                    @foreach ($categories as $item)
                                        @if (is_object($product->category))
                                            @php
                                                $value_ids = [];
                                                foreach ($product->category as $key => $item2) {
                            ...
                            <div class="form-group">
                                ...
                                <select name="sub_category_id" id="sub_category" multiple="multiple" class="form-control select2 select2-multiple">
                                    ...
                                    @foreach ($sub_categories as $key=>$item)
                                        @if (is_object($product->sub_category))
                                            @php
                                                $value_ids = [];
                                                foreach ($product->sub_category as $key => $item2) {
                                                    array_push($value_ids,$item2->id);
                                                }
                                            @endphp
                                        @endif
                    ...
                    <div class="row">
                        <div class="col-md-6 ">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="description" value="{{$product->description}}" rows="4"></textarea>
                            </div>
                        </div>
                        <div class="col-md-6 ">
                            <div class="form-group">
                                <h5 class="card-title m-t-40">Product Features</h5>
                                <textarea class="form-control" name="features" value="{{$product->features}}" rows="4"></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div>
                                    <img src="/{{$product->thumb_image}}" height="50" width="50">
                                </div>
                                ...
                                <table class="table table-bordered td-padding">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                                <select name="color_id" id="" class="form-control select2 select2-multiple" multiple="multiple">
                                                    ...
                                                    @if (is_object($product->color))
                                                        @php
                                                            $value_ids = [];
                                                            foreach ($product->color as $key => $item2) {
                                                                array_push($value_ids,$item2->id);
                                                            }
                                                        @endphp
                                                    @endif
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <select name="size_id" id="" class="form-control select2 select2-multiple" multiple="multiple">
                                                    ...
                                                    @if (is_object($product->size))
                                                        @php
                                                            $value_ids = [];
                                                            foreach ($product->size as $key => $item2) {
                                                                array_push($value_ids,$item2->id);
                                                            }
                                                        @endphp
                                                    @endif
                                            ...
                                            <td>
                                                <select name="unit_id" id="" class="form-control select2 select2-multiple" multiple="multiple">
                                                    ...
                                                    @if (is_object($product->unit))
                                                        @php
                                                            $value_ids = [];
                                                            foreach ($product->unit as $key => $item2) {
                                                                array_push($value_ids,$item2->id);
                                                            }
                                                        @endphp
                                                    @endif
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <select name="writer" id="" class="form-control select2 select2-multiple" multiple="multiple">
                                                                                      
                                                    @foreach ($writers as $item)
                                                        @if (is_object($product->writer))
                                                            @php
                                                                $value_ids = [];
                                                                foreach ($product->writer as $key => $item2) {
                                                                    array_push($value_ids,$item2->id);
                                                                }
                                                            @endphp
                                                        @endif
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <select name="publication" id="" class="form-control select2 select2-multiple" multiple="multiple">
                                                   
                                                    @foreach ($publications as $key=>$item)
                                                        @if (is_object($product->publication))
                                                            @php
                                                                $value_ids = [];
                                                                foreach ($product->publication as $key => $item2) {
                                                                    array_push($value_ids,$item2->id);
                                                                }
                                                            @endphp
                                                        @endif
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="number" step="0.01" class="form-control" name="tax" value="{{$product->tax}}">
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <option {{$product->free_delivery == 'On' ? 'selected' : ''}} value="On">On</option>
                                                <option {{$product->free_delivery == 'Off' ? 'selected' : ''}} value="Off">Off</option>
                                                </select>
                                            </td>
                                        </tr>
